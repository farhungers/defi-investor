...
- [x] Apply Migration 010 to Supabase — applied 2026-08-03. All three tables verified empty via smoke query. Comment stripping needed on first attempt (Supabase editor choked on `--` line containing `|`).
...
| 2026-08-03 | Phase 3e seeder re-run | `scripts/seed_venue_coin_map.py` re-fired against prod: 250 rows upserted. Bitget 415 direct + 3 prefix alias + 125 absent; Binance 421 direct + 122 absent. `__ABSENT__:{coin}` sentinels now populated so `capture_daemon` skips venue-absent coins on subscription. |
| 2026-08-03 | Phase 3e daemon fix | `capture_daemon` was capping `--max-symbols` before applying `--venues` filter. Surfaced by dry-run smoke: `--venues bitget --max-symbols 3` yielded 0 subs when the top-3 universe entries were Bitget-absent. Extracted pure `_select_universe(entries, venues, max_symbols)` helper (filter then cap); 4 new regression tests. Re-smoked live: 3 Bitget subs + ack. 304/304. Commit `99d193c`. |
| 2026-08-03 | Phase 3e Migration 010 applied | User pasted `db/migrations/010_orderbook.sql` (comment-stripped variant — Supabase editor rejected the `--` line containing `|`) into the SQL editor. Three tables live and empty: `orderbook_snapshots_l2`, `orderbook_features`, `orderbook_universe`. capture_daemon now writes to a real target when run without `--dry-run`. |
| 2026-08-13 | Phase 3e hardening pass | Session 6 (Kepler). Local audit for suffix-blind equality + cap-before-filter repeat instances came up clean. Two audit-found weak spots fixed: (1) `test_drop_oldest_survives_active_drain` closes the drain-loop x drop-oldest coverage gap in `test_orderbook_storage.py`; (2) `gate_report_a2b._load_labels_for_horizon` switched from `.eq()` to `.like(f"{prefix}%")` mirroring the backfill fix (defense-in-depth against a future retry-suffixed variant). New `.github/workflows/orderbook_cleanup.yml` (hourly at :17, 24h retention) wires the free-tier pg_cron substitute so the daemon can land data without blowing quota. 305/305. Not yet pushed; VM daemon launch still pending user action. |
